...
10. **목업 우선 — 충돌 3종 목업대로 복원** 🔄 (v3 후속, 사용자 "내가 보낸거 우선")
    - **면접 인라인 결과 패널 + 수동 진행**: 답변 종료 → `lastResult`에 결과 저장 → `ResultPanel`(전사 + 점수 도넛 3 + 강점/개선점 칩 + "다음 질문 →"/"종합 피드백 보기 →"). 자동 진행(`scheduleAdvance`/타이머) 제거 — 사용자가 결과 확인 후 직접 진행.
    - **"지난 면접 기록" 사이드 섹션 복원**: 신규 `services/pastInterviews.js` 샘플(네이버·백엔드 78 / 삼성 SDS·풀스택 71). `FeedbackList`에 `pastRecords` 섹션 추가, 클릭 시 동일 상세 모달.
    - **강점 재노출**: 결과 패널·상세 모달(도넛 옆 강점/개선점 칩)·종합 뷰(질문별 칩)에 강점/개선점 표시.
    - 검증: lint 0 / 테스트 12 / build 328모듈 / 렌더 스모크 4.
