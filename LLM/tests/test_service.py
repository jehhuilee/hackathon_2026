...
from LLM.service import evaluate_answer, generate_followups, generate_questions
...
class ServiceTests(unittest.TestCase):
    ...
    @patch("LLM.service.complete")
    def test_generate_followups_for_c_difficulty(self, mock_complete):
        mock_complete.return_value = (
            '[{"keyword": "파이프라인 최적화", '
            '"question": "최적화 전후 처리량을 수치로 말씀해 주실 수 있나요?", '
            '"intent": "정량적 근거 검증"}]'
        )
        ...
        result = generate_followups(data)

        self.assertEqual(len(result), 1)
        self.assertEqual(result[0]["id"], "f1")
        self.assertEqual(result[0]["category"], "꼬리 질문")
        self.assertEqual(result[0]["keyword"], "파이프라인 최적화")
        self.assertIn("처리량", result[0]["question"])

    @patch("LLM.service.complete")
    def test_generate_followups_skipped_for_easy_difficulty(self, mock_complete):
